...
def exercise_two():
    ...
    th2 = cv2.adaptiveThreshold(image, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 21, 10)
    display_image(th2)
...
def exercise_four():
    ...
    while True:
        ...
        if cv2.waitKey(10) & 0xFF == ord('q'):  # Break loop when the "q" is pressed on the keyboard
            break
